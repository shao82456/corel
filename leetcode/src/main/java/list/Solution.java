...
//        }
    ...
    public Node flatten1(Node head) {
        if (head.child != null) {
            ...
            return childLast.next == null ? childLast : flatten1(childLast.next);
     ...
     * 转换为数组
     * 利用sum[i,j]=sum[j]-sum[i]+num[i] O(1)算出sum
     * O(N^2) 穷举所有的子数组
     *
     * @param head
     * @return
    ...
    public ListNode removeZeroSumSublists(ListNode head) {
        ...
        for (ListNode cur = head; cur != null; cur = cur.next) {
            ...
            data.add(last + cur.val);
        }
        boolean[] toRemove = new boolean[arr.size()];
        /*bruce search*/
        for (int i = 0; i < arr.size(); i++) {
            for (int j = i; j < arr.size(); j++) {
                if(!toRemove[i]&&!toRemove[j]){
                    int sum = data.get(j) - data.get(i) + arr.get(i).val;
                    if (sum == 0) {
                        Arrays.fill(toRemove,i,j+1,true);
                    }
                }
            }
        }
