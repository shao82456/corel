    ...
    * 先用回溯看下*/
    ...
    public int maxUncrossedLines(int[] A, int[] B) {
        ...
        for(int i=0;i<B.length;i++){
            ArrayList<Integer> q=b.getOrDefault(B[i],new ArrayList<>());
            q.add(i);
            b.put(B[i],q);
        }
        maxUncrossedLines(0,A,b,-1,0);
        return ul;
    }

    private void maxUncrossedLines(int i,int[] A, Map<Integer,ArrayList<Integer>> b,int mark,int cur) {
        if(i==A.length){
            ul=Math.max(ul,cur);
            return;
        }
        if(b.containsKey(A[i])){
            ArrayList<Integer> arr=b.get(A[i]);
            int next=-1;
            for(int n:arr){
                if(n>mark){
                    next=n;
            ...
            if(next!=-1){
                cur+=1;
                maxUncrossedLines(i+1,A,b,next,cur);
                cur-=1;
            }
        }
        maxUncrossedLines(i+1,A,b,mark,cur);
    }

    /*贪心,
    * 看来不能完全贪心，可以选择连或者不连*/
    public int maxUncrossedLinesII(int[] A, int[] B) {
        int res=0;
        Map<Integer,Queue<Integer>> b=new HashMap<>();
        for(int i=0;i<B.length;i++){
            Queue<Integer> q=b.getOrDefault(B[i],new LinkedList<>());
            q.add(i);
            b.put(B[i],q);
        }
        int marked=-1;
        for(int i=0;i<A.length&&marked!=B.length-1;i++){
//            System.out.println(i+":"+marked+":"+res);
            if(b.containsKey(A[i])){
                Queue<Integer> q=b.get(A[i]);
                while (!q.isEmpty()&&q.peek()<=marked){
                    q.poll();
                }
                if(!q.isEmpty()){
                    marked=q.poll();
    ...
    * 但是题目就故意搞这么一个case*/
    ...
    private boolean canBreak(String s, List<String> wordDict) {
        ...
        for(String part:wordDict){
            if(s.startsWith(part)&&canBreak(s.substring(part.length()),wordDict)){
                res=true;
            }
        }
        return res;
